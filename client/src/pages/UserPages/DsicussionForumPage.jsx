...
const DiscussionForumPage = () => {
  ...
  if (loading) {
    return (
      <div className={`flex justify-center items-center min-h-screen ${isDarkTheme ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500" />
      </div>
    );
  }
  ...
  return (
    <div className={isDarkTheme ? 'bg-gray-900' : 'bg-white'}>
      <UserHeader />
      <div className={`max-w-2xl mx-auto p-4 space-y-6 ${isDarkTheme ? 'bg-gray-900' : 'bg-gray-50'} min-h-screen mt-10`}>
        <h1 className={`text-3xl font-bold text-center mb-6 ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>
          Discussions
        </h1>
        
        {user && (
          <div className={`${isDarkTheme ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm p-4 space-y-4`}>
            <div className="flex items-center space-x-4">
              <img src="download.png" alt="Your avatar" className="w-12 h-12 rounded-full" />
              <input
                type="text"
                value={newPost}
                onChange={(e) => setNewPost(e.target.value)}
                placeholder="Start a discussion..."
                className={`flex-grow p-2 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  isDarkTheme ? 'bg-gray-700 text-white placeholder-gray-400' : 'bg-gray-100 text-gray-700'
                }`}
              />
            </div>
            <div className="flex justify-end">
        ...
        {posts.map(post => (
          <div key={post.id} className={`${isDarkTheme ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm p-4 space-y-4`}>
            <div className="flex justify-between items-start">
              <div className="flex space-x-3">
                <img src="download.png" alt={post.author} className="w-10 h-10 rounded-full" />
                <div>
                  <h3 className={`font-semibold ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>{post.author}</h3>
                  <p className={`text-xs ${isDarkTheme ? 'text-gray-400' : 'text-gray-500'}`}>
                    {formatDistanceToNow(new Date(post.createdAt), { addSuffix: true })}
                  </p>
                  <p className={`mt-1 ${isDarkTheme ? 'text-gray-300' : 'text-gray-600'}`}>{post.content}</p>
                </div>
              </div>
              {user?.uid === post.authorId && (
                <button 
            ...
            <div className="pl-12 space-y-4">
              {post.replies?.map(reply => (
                <div key={reply.id} className="flex space-x-3">
                  <img src="download.png" alt={reply.author} className="w-8 h-8 rounded-full" />
                  <div className="flex-grow">
                    <div className={`rounded-2xl p-3 ${isDarkTheme ? 'bg-gray-700' : 'bg-gray-100'}`}>
                      <h4 className={`font-semibold ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>{reply.author}</h4>
                      <p className={`text-xs ${isDarkTheme ? 'text-gray-400' : 'text-gray-500'}`}>
                        {formatDistanceToNow(new Date(reply.createdAt), { addSuffix: true })}
                      </p>
                      <p className={`mt-1 ${isDarkTheme ? 'text-gray-300' : 'text-gray-600'}`}>{reply.content}</p>
                    </div>
                    <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                      <button 
                        onClick={() => likeReply(post.id, reply.id)}
            ...
            {user && (
              <div className="flex items-center space-x-3 pl-12">
                <img src="download.png" alt="Your avatar" className="w-8 h-8 rounded-full" />
                <input
                  type="text"
                  value={replyContents[post.id] || ''}
                  onChange={(e) => setReplyContents({ ...replyContents, [post.id]: e.target.value })}
                  placeholder="Write a reply..."
                  className={`flex-grow p-2 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    isDarkTheme ? 'bg-gray-700 text-white placeholder-gray-400' : 'bg-gray-100 text-gray-700'
                  }`}
                />
                <button
        ...
        {posts.length === 0 && (
          <div className={`text-center ${isDarkTheme ? 'text-gray-400' : 'text-gray-500'} mt-12`}>
            <MessageSquare className="w-16 h-16 mx-auto mb-4 text-gray-400" />
            <p className="text-xl font-semibold">No discussions yet</p>
            <p className="mt-2">Be the first to start a conversation!</p>
          </div>
